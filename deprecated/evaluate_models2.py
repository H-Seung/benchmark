...
def get_process_gpu_mem_mb(device_index: int = 0) -> float:
    """
    Return current GPU memory used by *this process* in MB (NVML).
    If per-process query fails, fall back to total device used memory.
    """
    if not _NVML_AVAILABLE:
    ...
    try:
        ...
        # Prefer per-process memory
        # Compute 프로세스와 Graphics 프로세스 리스트를 모두 가져옴
        procs = []
        try:
        ...
        for p in procs:
            if int(p.pid) == int(pid):
                print("Matched current PID")
                # usedGpuMemory is in bytes
                return float(p.usedGpuMemory) / (1024 ** 2)

        # Fallback: total device used memory.
        print("PID not found in compute list. Fallback to total mem.")
...
def measure_pipeline_latency_split(model: YOLO, images, imgsz: int):
    ...
    - total: sum of above (excluding python overhead around it)
    Also measures VRAM peak via NVML (process GPU mem) + torch allocator peak for reference.
    """
    # Warmup (use existing end-to-end call)
    ...
    # Predictor init (use a real file if assets unavailable)
    ...
    with torch.no_grad():
        for img_path in images[PIPE_WARMUP:]:
            ...
            # 2) Preprocess (letterbox/normalize + to tensor + move to device)
            im0s = [im0]
            ...
            # Update NVML peak
            vnow = get_process_gpu_mem_mb(DEVICE)
            print(f"Current process GPU memory usage: {vnow:.2f} MB")
            if not np.isnan(vnow):
                vram_nvml_peak = max(vram_nvml_peak, vnow)
    ...
    # Torch allocator peak (reference; TRT/ORT allocations may not fully show here)
    vram_torch_peak = torch.cuda.max_memory_allocated() / (1024 ** 2)

    if vram_nvml_peak == float("-inf"):
        vram_nvml_peak = float("nan")
    ...
    return {
        ...
        "vram_nvml_peak_mb": vram_nvml_peak,
    ...
    for model_file in model_files:
        ...
        for current_imgsz in sizes_to_test:
            ...
            row = {
                ...
                # VRAM: NVML peak (real), plus torch peak (reference)
                "vram_nvml_peak_mb": None if np.isnan(pipe["vram_nvml_peak_mb"]) else round(pipe["vram_nvml_peak_mb"], 1),
